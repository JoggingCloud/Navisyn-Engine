...
{
	m_position = { listenerPosition.x, listenerPosition.y, listenerPosition.z };
	m_forward = { listenerForward.x, listenerForward.y, listenerForward.z };
	m_up = { listenerUp.x, listenerUp.y, listenerUp.z };
	m_fmodSystem->set3DListenerAttributes(listenerIndex, &m_position, nullptr, &m_forward, &m_up);
}
...
void AudioSystem::SetSoundPosition(SoundPlaybackID soundPlaybackID, const Vec3& soundPosition)
{
	...
	{
		FMOD::Channel* channel = (FMOD::Channel*)soundPlaybackID;
		m_position = { soundPosition.x, soundPosition.y, soundPosition.z };
		channel->set3DAttributes(&m_position, nullptr);
	}
}
...
{
	if (soundID < 0 || static_cast<size_t>(soundID) >= m_registeredSounds.size())
	{
	...
	if (channelAssignedToSound)
	{
		...
		FMOD_MODE playMode = isLooped ? FMOD_LOOP_NORMAL : FMOD_LOOP_OFF;
		playMode |= FMOD_3D;
		channelAssignedToSound->setMode(playMode);
		channelAssignedToSound->set3DAttributes(&m_position, nullptr);
...
SoundID AudioSystem::CreateOrGetSound( const std::string& soundFilePath, bool is3D )
{
	...
	else
	{
		...
		// Use FMOD_3D for 3D sounds or FMOD_DEFAULT for 2D sounds
		FMOD_RESULT result = m_fmodSystem->createSound( soundFilePath.c_str(), mode, nullptr, &newSound );
		if( result == FMOD_OK && newSound )
		{
			SoundID newSoundID = m_registeredSounds.size();
			m_registeredSoundIDs[ soundFilePath ] = newSoundID;
			m_registeredSounds.emplace_back( newSound );
			return newSoundID;
		}
	}
