...
#define VERIFY_RECOVERABLE(condition, reason)                                         \
    do {                                                                              \
        bool __condResult = static_cast<bool>(condition);							  \
        if (!__condResult) {														  \
            RecoverableWarning(__FILE__, __FUNCTION__, __LINE__, reason, #condition); \
        }                                                                             \
    } while (0)